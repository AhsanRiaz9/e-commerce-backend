...
@router.post("/products/", response_model=products_schema.Product)
def create_product_route(product: products_schema.ProductBase, db: Session = Depends(get_db)):
    return create_product(db, product)

@router.get("/products/{product_id}/", response_model=products_schema.Product)
...
@router.put("/products/{product_id}/", response_model=products_schema.Product)
def update_product_route(product_id: int, product: products_schema.ProductBase, db: Session = Depends(get_db)):
    db_product = get_product(db, product_id)
    if db_product is None:
        raise HTTPException(status_code=404, detail="Product not found")
